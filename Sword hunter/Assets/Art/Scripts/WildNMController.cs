...
public class WildNMController : MonoBehaviour
{
    int pcoin;
    ...
    void Update()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(rigidbody2d.position + Vector2.up * 0.2f, directionX, 1.5f, LayerMask.GetMask("Character"));
        ...
        if (cham)
        {
            ChangeHealthWild(-damageReceived);
        }
    }
    ...
    public void ChangeHealthWild(int amount)
    {
        if (amount < 0)
        {
            if (isInvincible)
